...
const NextMilestoneLoader = ({alignmentOnly, books, bookNo, previous} ) => {
  ...
  async function loadNextMs () {
    ...
    let versionCode = bookNo === 1 ? books.book1.versionCode : books.book2.versionCode;
    let milestoneTexts = bookNo === 1 ? books.book1.content : books.book2.content;
    if (milestoneTexts === undefined || Object.keys(milestoneTexts).length === 0) {
      if (downloadedTexts?.[releaseCode]?.[versionCode]?.["downloadedMs"]){
        milestoneTexts = downloadedTexts[releaseCode][versionCode]["downloadedMs"]["msTexts"];
        //console.log(milestoneTexts); 
      } 
    }
    const currentMs = bookNo === 1 ? books.book1.ms : books.book2.ms;
    let allDisplayedMsNos = Object.keys(bookNo === 1 ? displayMs.book1 : displayMs.book2).map(Number);
    ...
    const allMsNos = previous 
      ? Object.keys(milestoneTexts)
          .sort((n1, n2) => parseInt(n1) - parseInt(n2)).reverse()
      : Object.keys(milestoneTexts)
    ...
    const msText = await getMilestoneText(
      releaseCode, versionCode, msNo, downloadedTexts, setDownloadedTexts
    );
    setDisplayMs((prev) => {
      let newDisplayMs = {...prev}; // this is necessary for the ref to the object to change, triggering a rerender!
      if (bookNo === 1) {
